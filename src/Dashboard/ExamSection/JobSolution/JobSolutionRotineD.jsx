...
const JobSolutionRotineD = () => {
  ...
  const handleDelete = (_id) => {
    fetch(`http://localhost:5000/Job-Solution-rutin-delete?_id=${_id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    const url = `http://localhost:5000/Job-Solution-rutin`;
    fetch(url)
      .then((res) => res.json())
  ...
  return (
    <div>
      <h1 className="text-center my-5 ">Job Solution Rutine</h1>
          ...
          {rutin.map((rutins) => {
            return (
              <div className="text-center p-5 border border-dashed h-40 rounded">
                ...
                <small>
                  ফ্রী পরীক্ষা প্রতি শুক্রবার সকলের জন্য
                </small>
              </div>
            );
          })}
...
export default JobSolutionRotineD;
