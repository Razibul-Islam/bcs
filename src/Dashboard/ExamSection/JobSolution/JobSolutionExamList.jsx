...
const JobSolutionExamList = () => {
  ...
  const handleDelete = (_id) => {
    const url = `http://localhost:5000/get-Job-Solution-exam-one?_id=${_id}`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => setDeletedQuestion(data));

    if (deletee) {
      fetch(`http://localhost:5000/delete-Job-Solution-modeltest?_id=${_id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
...
export default JobSolutionExamList;
