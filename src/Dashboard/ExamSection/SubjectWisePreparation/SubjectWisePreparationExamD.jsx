...
const SubjectWisePreparationExamD = () => {
  ...
  const handleDelete = (_id) => {
    const url = `http://localhost:5000/subject-wise-exam-one?_id=${_id}`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => setDeletedQuestion(data));

    if (deletee) {
      fetch(`http://localhost:5000/delete-subject-wise-modeltest?_id=${_id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  useEffect(() => {
    const url = `http://localhost:5000/get-subject-wise-eaxma-all`;
    fetch(url)
      .then((res) => res.json())
