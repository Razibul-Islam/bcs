...
const PreparationForGreadMerit = () => {
    const [result, setresult] = useState([]);
    const [user, loading, error] = useAuthState(auth);

    useEffect(() => {
        const url = `https://bcspioneer.vercel.app/grade-preparation-result?userEmail=${user?.email}`;
        fetch(url)
            .then((res) => res.json())
            ...
            {result.map((res) => {
                return (
                    <div className="text-center p-5 border border-dashed ">
                        ...
                        <Link
                            to={`/preparationforgreadMerit/${res.examDate}`}
...
export default PreparationForGreadMerit;