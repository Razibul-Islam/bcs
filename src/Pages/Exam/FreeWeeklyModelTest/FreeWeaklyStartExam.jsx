                    ...
                    question?.examQuestion?.map(qsn => {
                        return (
                            <div className='my-5 bg-slate-100 '>
                                <h4 className='font-bold p-3'>{index++ + ''}){qsn.question}</h4>
                                <hr className='my-2' />
                                <div className='cursor-not-allowed' id={qsn._id}>
                                    <button onClick={() => handleChexkQuestion(qsn.ans, 'a', qsn._id)} id={qsn._id + 'a'} className='block py-3 px-5 hover:bg-teal-300 w-full text-start flex justify-start items-center gap-3'><span className='border border-black rounded-full h-6 w-6 block flex justify-center items-center   p-1'>ক</span> {qsn.opA}</button>
                                    <button onClick={() => handleChexkQuestion(qsn.ans, 'b', qsn._id)} id={qsn._id + 'b'} className='block py-3 px-5 hover:bg-teal-300 w-full text-start flex justify-start items-center gap-3'><span className='border border-black rounded-full h-6 w-6 block flex justify-center items-center   p-1'>খ</span> {qsn.opB}</button>
                                    <button onClick={() => handleChexkQuestion(qsn.ans, 'c', qsn._id)} id={qsn._id + 'c'} className='block py-3 px-5 hover:bg-teal-300 w-full text-start flex justify-start items-center gap-3'><span className='border border-black rounded-full h-6 w-6 block flex justify-center items-center   p-1'>গ</span> {qsn.opC}</button>
                                    <button onClick={() => handleChexkQuestion(qsn.ans, 'd', qsn._id)} id={qsn._id + 'd'} className='block py-3 px-5 hover:bg-teal-300 w-full text-start flex justify-start items-center gap-3'><span className='border border-black rounded-full h-6 w-6 block flex justify-center items-center   p-1'>ঘ</span> {qsn.opD}</button>
                                </div>
                            </div>
                        )
                    })
